...
if __name__ == "__main__":
    # 测试子任务代理
    async def test_sub_task_agent():
        agent = SubTaskAgent(name="TestSubTaskAgent")
        message = Message(
            content={
                "document": "这是一个测试文档，包含一些信息。",
                "task": "提取文档中的所有命名实体。"
            },
            sender="User",
            recipient="TestSubTaskAgent"
        )
        result = await agent.process(message)
        print(f"子任务处理结果: {result.content}")

    asyncio.run(test_sub_task_agent())